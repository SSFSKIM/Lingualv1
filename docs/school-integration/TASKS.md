...
- [-] Add per-route-family read feature flags so a failed cut can toggle reads back to Firestore without redeploy (TECH_SPEC §3.8e). **Designed (2026-06-01) — see `READ_CUTOVER.md` §2.** A `ReadRouter` wraps the `database` module on `deps.db` (overrides only cut-over readers; `__getattr__` passes everything else through → zero route edits). Per-entity flags `READ_PG_{ORGANIZATIONS,MEMBERSHIPS,CLASSES,ENROLLMENTS}`, default OFF, 3-state (`0`/`shadow`/`1`), fail-open to Firestore. Implementation pending the prereqs below.
...
- [-] Cut route reads over by family. **Scoped + designed + adversarially reviewed (2026-06-01) — authoritative sequencing in `READ_CUTOVER.md`.** Candidates are ONLY the 4 dual-written entities (org/membership/class/enrollment); the **analytics family is BLOCKED** (practice_sessions/learning_events are modeled but NOT dual-written/backfilled — see the `[!]` item below). Ordered slices:
  - [ ] **(1) enrollments-read-prereq** *(code-only, flag OFF)* — fix confirmed defect **D1**: the enrollment twin (`repository/enrollments.py:48`) emits `class_id` as the PG UUID, which breaks `list_student_classes`/practice-launch lookup after a flip. Serializers must emit FKs as `legacy_firestore_id` via JOIN (new serializer invariant, `READ_CUTOVER.md` §3.0). Add `count_org_students`. No flip.
  - [ ] **(2) organizations-read** + flag — full org shape incl. derived `school_admin_uids` (defect **D3**: `memberCount` regresses otherwise) and the missed reader `get_org_by_teacher_invite_code` (§3.0a); cut the org-doc family atomically.
