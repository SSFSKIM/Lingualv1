...
- [-] Cut route reads over by family. **Scoped + designed + adversarially reviewed (2026-06-01) — authoritative sequencing in `READ_CUTOVER.md`.** Candidates are ONLY the 4 dual-written entities (org/membership/class/enrollment); the **analytics family is BLOCKED** (practice_sessions/learning_events are modeled but NOT dual-written/backfilled — see the `[!]` item below). Ordered slices:
  ...
  - [-] **(2) organizations-read** — INFRA + first adapter landed (2026-06-01, flag OFF): `backend/db/repository/organizations_read.py` `get_organization` (full org shape, inverse `suspended_by_uid`/`restored_by_uid` renames, `school_admin_uids` omitted+allowlisted) + the `ReadRouter.get_organization` override + shadow parity compare. **Remaining before flipping `READ_PG_ORGANIZATIONS=1`:** the other org readers for entity-atomicity (`search_organizations`, `list_organizations` incl. derived `school_admin_uids`/`memberCount` — defect **D3**, `count_organizations_by_status`, the missed `get_org_by_teacher_invite_code` — §3.0a); then deploy with `=shadow` + soak. Shadow-capable now.
