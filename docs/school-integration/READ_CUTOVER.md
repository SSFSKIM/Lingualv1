...
9. **analytics — DESIGN LANDED 2026-06-02** (was blocked by the sessions/events boundary, §0). See **`ANALYTICS_MIGRATION.md`** (verdict GO-WITH-FIXES, 5 flag-gated slices: assignments dual-write → practice_sessions dual-write → learning_events session-close batch via a separate Cloud Run Job → read-shadow → flip + Firestore-write retirement). Still 100% Firestore until those slices ship; unblocks the §0 boundary when they do.
