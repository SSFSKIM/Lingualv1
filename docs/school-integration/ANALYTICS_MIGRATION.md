...
Date: 2026-06-03 | Status: **Slice A (assignments) SHIPPED & PG-AUTHORITATIVE. Slice B (practice_sessions dual-write) SHIPPED & LIVE. Slice C (learning_events dual-write) SHIPPED & LIVE in prod (2026-06-03, rev `lingual-app-00058-gx4`, `DUAL_WRITE_ANALYTICS_EVENTS=1`): `shadow_write_turn` + the §5b.2-#7 self-disable live; events term-scope backfill **1401 inserted / 0 errors**; Pass 2 per-session event-COUNT parity **`in_sync`, fs 1401 = pg 1401, 0 short / 0 surplus**. Slice D (analytics read adapters + split-pass parity) SHIPPED & PG-AUTHORITATIVE — **ALL ANALYTICS READS NOW SERVE FROM POSTGRES** (2026-06-03): SESSION-read flipped rev `00059-msz` (`READ_PG_ANALYTICS_SESSIONS=1`), EVENT-read flipped rev `00061-29z` (`READ_PG_ANALYTICS_EVENTS=1`), both fail-open. **The entire read cutover is complete** (org/class/enrollment/assignment/practice_sessions/learning_events all PG; memberships shadow by design, D4). **Verification:** Pass 1 session_summary parity `in_sync` 49/49 0 mismatched (incl. 1 LIVE non-backfilled smoke-test session — `shadow_write_turn` proven end-to-end across session-create/events/finalize seams), Pass 2 event-COUNT `in_sync` 1408=1408, live shadow compares (SESSIONS 1/0, EVENTS 1/0), post-flip teacher analytics PG-served (summary.totals + pedagogy event-aggregations) with 0 fail-open. Alembic `0003` payload GIN applied to prod (`alembic upgrade head` as postgres owner; backs the context_tag/error aggregations). **Slice E COMPLETE — `WRITE_FIRESTORE_ANALYTICS=0` FLIPPED 2026-06-03 (rev `lingual-app-00065-ldh`): Firestore analytics writes RETIRED, Postgres is the SOLE store for practice_sessions + learning_events. THE ANALYTICS MIGRATION IS DONE.** Deploy chain reached 00062 (retirement code, behavior-neutral) → 00063/00064 (reconciler secret mounted) → 00065 (`=0` flip). Startup banner confirmed "Firestore analytics writes RETIRED; Postgres is the sole store (fail-closed)", 0 warnings. Smoke-test proof of the PG-only fail-closed path: authenticated POST create-session (assignment `xnVbMFd4GG6H74QKRaE2`) → 201 (read-after-write body) → POST `student.turn` → 200; PG sessions 49→50, session `SBub1z0xuZXPi0qk7iLG` active w/ 2 events, Firestore doc ABSENT + 0 Firestore events. Orphaned-session reconciler LIVE (hourly Cloud Scheduler `analytics-sweep-orphaned-sessions`). Rollback instant: `--update-env-vars WRITE_FIRESTORE_ANALYTICS=1`. Build detail below: ** Build: `dual_write_analytics.{primary_create_practice_session, primary_update_practice_session, primary_write_turn, write_turn dispatcher, _run_with_timeout_strict, firestore_analytics_enabled}` (fail-CLOSED — PG sole store, exceptions propagate to a 500 the SPA retries; shared `_apply_turn`/`_prepare_turn` core with the shadow, `strict` selects raise-vs-silent-drop on unresolved parent); `database.{create_practice_session, update_practice_session, create_learning_event}` skip the Firestore write when `'0'` and mint Firestore-style ids client-side via `.document().id` (no network write, so `legacy_firestore_id` stays Firestore-style); route calls `write_turn` (dispatch). Read-after-write: new `analytics_reads.get_practice_session` point-get + ReadRouter override (gated `READ_PG_ANALYTICS_SESSIONS` also `READ_PG_ASSIGNMENTS`, `_FALLBACK` on absent) so the 5 `get_practice_session` callers (curriculum_admin read-back/validation/serialize + chat.py voice/text validation) resolve PG-only sessions. cloudbuild `_WRITE_FIRESTORE_ANALYTICS` default `'1'` + main.py banner (warns if `'0'` without READ_PG_ANALYTICS_SESSIONS=1 / DUAL_WRITE_ANALYTICS_EVENTS=1). 1218 backend green. **Flip prereqs (HARD):** READ_PG_ANALYTICS_SESSIONS=1 (live) + DUAL_WRITE_ANALYTICS_EVENTS=1 (live) + a clean confidence sprint at `'1'`. Flip = `--update-env-vars WRITE_FIRESTORE_ANALYTICS=0`; rollback instant via `=1`.** Deploy chain: 00057 (C+D dormant) → 00058 (events dual-write + session-read shadow) → 00059 (session-read `=1`) → 00060 (event-read shadow) → 00061 (event-read `=1`).**

> **Slice B SHIPPED & LIVE 2026-06-02 (rev `lingual-app-00056-kmv`).** Live rollout: cloudbuild flag-default audit (no `--set-env-vars` regression) → deploy rev 00055 (flag '0', clean boot) → `--update-env-vars DUAL_WRITE_ANALYTICS_SESSIONS=1` rev 00056 (env verified on serving rev) → term backfill `--write` (practice_sessions **48 inserted / 0 errors**) + `--parity` (**`in_sync`, fs 48 = pg 48, 0 missing/0 extra**) → cloudbuild default bumped 0→1. Confirmed: rev 00056 startup banner `DUAL_WRITE_ANALYTICS_SESSIONS=1`, 0 fail-open. Forward live session-create not yet observed (no student traffic since flip) — fail-open + the write path is prod-proven (the backfill runs the same `upsert_practice_session` the shadow calls). Reconciler LIVE 2026-06-03 (`internal-scheduler-secret` + hourly Cloud Scheduler job `analytics-sweep-orphaned-sessions`, verified HTTP 200, first run swept 19; see LIMITATIONS #48(c)). Implementation detail follows:
