...
Date: 2026-06-03 | Status: **Slice A (assignments) SHIPPED & PG-AUTHORITATIVE. Slice B (practice_sessions dual-write) SHIPPED & LIVE. Slice C (learning_events dual-write) SHIPPED & LIVE in prod (2026-06-03, rev `lingual-app-00058-gx4`, `DUAL_WRITE_ANALYTICS_EVENTS=1`): `shadow_write_turn` + the §5b.2-#7 self-disable live; events term-scope backfill **1401 inserted / 0 errors**; Pass 2 per-session event-COUNT parity **`in_sync`, fs 1401 = pg 1401, 0 short / 0 surplus**. Slice D (analytics read adapters + split-pass parity) DEPLOYED; SESSION-read shadow SOAKING (`READ_PG_ANALYTICS_SESSIONS=shadow`, rev 00058); Pass 1 session_summary field parity **`in_sync`, 48/48, 0 mismatched** (point-in-time). EVENT-read still `'0'` (gated on the GIN before perf matters). Slice E (read FLIPS to `'1'` + Firestore write retirement) STARTED — both parity gates clean on full term scope, **48h live shadow soak now running** before any `=1` flip. Reads remain 100% Firestore-authoritative (`shadow` = compare-only). Alembic `0003` GIN NOT yet applied (needed before the EVENT read flip, not before the soak). Deploy chain: rev 00057 (C+D dormant) → rev 00058 (`DUAL_WRITE_ANALYTICS_EVENTS=1` + `READ_PG_ANALYTICS_SESSIONS=shadow`).**
